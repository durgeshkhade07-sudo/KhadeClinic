...
def book():
    ...
    if not patient:
        patient = Patient(name=data['name'], mobile=data['mobile'], age=data['age'], gender=data['gender'])
...
def get_apps():
    ...
    return jsonify([{"id": a.id, "token": a.token_number, "name": p.name, "age": p.age, "gender": p.gender, "dept": a.department} for a, p in apps])
...
def generate_pdf(token):
    ...
    pdf.cell(200, 25, txt="KHADE CLINIC", ln=True, align='C')
    ...
    pdf.output(f"prescriptions/token_{token}.pdf")
